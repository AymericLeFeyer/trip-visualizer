...
> Dernière mise à jour : 2026-07-23 (créneau + tri chrono des lieux, images lieux/étapes, infos confidentielles, vue jour par jour, tiroir desktop)
...
- **Détail/édition : tiroir desktop, modale mobile** : le contenu (éditeur si `isAdmin`, sinon vue lecture seule) est factorisé dans `DetailContent` (`panels/`, aiguilleur stage/place/leg/flight). Deux hôtes : `DetailDrawer` (**desktop**, `TripPage` : `<aside grid grid-rows-1 h-full>` entre la sidebar et la carte — la carte reste à droite ; largeur `w-[38rem]` pour une étape 2 colonnes, `w-[24rem]` sinon ; masqué pendant un placement) et `DetailModal` (**mobile**, primitive `ui/Modal`, pour l'édition déclenchée par les crayons). Cliquer un lieu **remplace** le contenu du tiroir (bouton retour vers l'étape dans les éditeurs). Éditeurs/détails = root `flex min-h-0 flex-col`, header `shrink-0`, corps `flex-1 min-h-0 overflow-y-auto`.
